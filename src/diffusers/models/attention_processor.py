...
class StableAudioAttnProcessor2_0:
    ...
    def __call__(
        ...
        # torch.size([10, 12, 130, 64])

        # MusicKV replacement
        if copy_blocks is not None and index_block in copy_blocks:
            mid = batch_size // 2
            key[1] = key[0] * (1 - t_ratio) + key[1] * (t_ratio)
            key[mid + 1] = key[mid] * (1 - t_ratio) + key[mid + 1] * (t_ratio)
            value[1] = value[0] * (1 - t_ratio) + value[1] * (t_ratio)
            value[mid + 1] = value[mid] * (1 - t_ratio) + value[mid + 1] * (t_ratio)
