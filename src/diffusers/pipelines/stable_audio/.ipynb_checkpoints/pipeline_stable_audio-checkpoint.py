...
class StableAudioPipeline(DiffusionPipeline):
    ...
    def encode_prompt(
        ...
        if prompt_embeds is None:
            ...
            diff_indices = torch.where(text_input_ids[0] != text_input_ids[-1])[0].tolist()
        ...
        return prompt_embeds, diff_indices
    ...
    def prepare_latents(
        ...
        if latents is None:
            latents = randn_tensor((num_channels_vae, sample_size), generator=generator, device=device, dtype=dtype).tile(batch_size, 1, 1)
        else:
    ...
    def __call__(
        ...
        prompt_embeds, diff_indices = self.encode_prompt(
            prompt,
            device,
            do_classifier_free_guidance,
            negative_prompt,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if USE_TRATIO:
                    t_ratio = 1-(i / len(timesteps))
                else:
                    t_ratio = 0
                self_ratio = t_ratio * (1 - beta)
                cross_ratio = t_ratio * (1 - gamma)
                # predict the noise residual
                if i <= (int(len(timesteps) * alpha)):
                ...
                if do_classifier_free_guidance:
                    ...
                    guidance_scales = torch.tensor([1, 9, 9], dtype=noise_pred_uncond.dtype, device=noise_pred_uncond.device).view(-1, 1, 1)
