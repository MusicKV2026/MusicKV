        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if do_classifier_free_guidance:
                    ...
                    if invert_audio:
                        guidance_scales = torch.tensor([1] + [guidance_scale] * (len(noise_pred_uncond)-1), dtype=noise_pred_uncond.dtype, device=noise_pred_uncond.device).view(-1, 1, 1)
                    else:
                        guidance_scales = torch.tensor([guidance_scale] * len(noise_pred_uncond), dtype=noise_pred_uncond.dtype, device=noise_pred_uncond.device).view(-1, 1, 1)
