...
class EDMDPMSolverMultistepScheduler(SchedulerMixin, ConfigMixin):
    ...
    def step_forward(
        ...
        """
        Reverse the diffusion process by adding noise back to the sample (inversion).
        This function propagates the sample backward with the multistep DPMSolver.
        
        Args:
            model_output (`torch.Tensor`):
                The direct output from learned diffusion model (not used in inversion).
            timestep (`int`):
                The current discrete timestep in the diffusion chain.
            sample (`torch.Tensor`):
                A current instance of a sample created by the diffusion process.
        ...
        Returns:
            [`~schedulers.scheduling_utils.SchedulerOutput`] or `tuple`:
                The sample tensor with added noise.
        """
        if self.num_inference_steps is None:
            raise ValueError(
                "Number of inference steps is 'None', you need to run 'set_timesteps' after creating the scheduler"
        ...
        # Determine the index for the reverse sigma schedule
        # Reverse timestep index: current_step = len(timesteps) - 1 - step_index
        if self.step_index is None:
            self._init_step_index(timestep)
        
        # For inversion, we need to use the sigma values in reverse order
        reverse_step_index = len(self.timesteps) - 1 - self.step_index
        
        # Calculate current and next sigma for inversion
        # Note: sigmas are stored in decreasing order (high to low noise)
        current_sigma = self.sigmas[self.step_index]
        next_sigma = self.sigmas[self.step_index + 1] if self.step_index < len(self.sigmas) - 1 else 0
        
        # Generate random noise with the same shape as sample
        noise = randn_tensor(
            sample.shape, 
            generator=generator, 
        ...
        # For inversion: x_{t} = x_{t-1} + sqrt(sigma_t^2 - sigma_{t-1}^2) * z
        noise_scaling = torch.sqrt(current_sigma**2 - next_sigma**2)
        
        # Apply noise to the current sample
        prev_sample = sample + noise_scaling * noise
        
        # Decrease step index for next backward step
        # (we're moving backward in the diffusion process)
        if self.step_index > 0:
            self.step_index -= 1
        else:
            # Reset state if we've completed the inversion
            self._step_index = None
