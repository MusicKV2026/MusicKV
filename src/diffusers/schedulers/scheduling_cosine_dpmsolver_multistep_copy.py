...
class CosineDPMSolverMultistepScheduler(SchedulerMixin, ConfigMixin):
    ...
    def set_timesteps(self, num_inference_steps: int = None, device: Union[str, torch.device] = None, invert_audio = False):
        """
    ...
    def step_forward(
        ...
        denoised = self.convert_model_output(model_output, sample=sample)
        
        # ==== 反向步进核心 ====
        # 获取当前噪声水平 (sigma_s) 和下一步更大的噪声水平 (sigma_t)
        sigma_t, sigma_s = self.sigmas[self.step_index + 1], self.sigmas[self.step_index]
        
        # 计算等效噪声 (从当前样本和去噪估计反推)
        noise = (sample - denoised) / sigma_s
        
        # 重构前一个噪声更重的状态 (加噪操作)
        prev_sample = denoised + noise * sigma_t
